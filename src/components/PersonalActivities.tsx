  ...
  return (
    <section className="bg-white dark:bg-slate-800 rounded-lg shadow-lg p-8">
      ...
      <div className="space-y-8">
        {personalActivities.blog.length > 0 && (
          <div>
            <h3 className="text-2xl font-semibold text-slate-800 dark:text-slate-200 mb-4">ブログ</h3>
            <div className="space-y-2">
              {personalActivities.blog.map((blog, index) => (
                <a 
                  key={index}
                  href={blog.url} 
                  target="_blank" 
        ...
        {personalActivities.speaking.length > 0 && (
          <div>
            ...
            <div className="space-y-4">
              {personalActivities.speaking.map((engagement, index) => (
                <div key={index} className="p-4 bg-slate-50 dark:bg-slate-700 rounded-lg">
                  <h4 className="font-semibold text-slate-800 dark:text-slate-200 mb-1">{engagement.event}</h4>
                  <p className="text-slate-600 dark:text-slate-300 mb-2">{engagement.title}</p>
                  <a href={engagement.url} target="_blank" rel="noopener noreferrer" className="text-blue-600 hover:text-blue-800 dark:text-blue-400 dark:hover:text-blue-300 text-sm">
                    資料を見る →
                  </a>
                </div>
              ))}
